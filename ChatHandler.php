...
class ChatHandler {
    ...
    public function doHandshake($received_header, $client_socket_resource, $host_name, $port) {
        ...
        foreach ($lines as $line) {
            $line = trim($line); // Use trim instead of chop
        ...
        if (!isset($headers['Sec-WebSocket-Key'])) {
            throw new Exception('WebSocket key missing in header.');
        }
    ...
    public function newConnectionACK($client_ip_address) {
        ...
        return $this->seal(json_encode($messageArray, JSON_UNESCAPED_SLASHES));
    }
